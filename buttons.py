...
def setting_button(update, context, sms):### Вывод кнопок Настроек
    reply_keyboard = [['Маницпуляции с героем'], ['Подписки...', 'Поменять время...'], ['Проверить данные профиля'], ['🔙Назад🔙']]
    bot.send_message(chat_id=update.effective_chat.id, text=sms,
...
def setting_hero_button(update, context, sms): # Манапуляции с героем
    ...
    if num == 5:
        reply_keyboard += [['Удалить одного героя'], ['Переименовать героя'], ['🔙Назад🔙']]
    elif num == 1:
        reply_keyboard += [['Добавить еще одного героя'], ['Переименовать героя'], ['🔙Назад🔙']]
    else:
        reply_keyboard += [['Добавить еще одного героя'], ['Удалить одного героя'], ['Переименовать героя'],
                           ['🔙Назад🔙']]
    bot.send_message(chat_id=update.effective_chat.id, text=sms,
                             reply_markup=ReplyKeyboardMarkup(reply_keyboard, resize_keyboard=True,
                                                              one_time_keyboard=False))
...
def Subscription_button(update, context ,sms): # Подписки...
    subscription = pd.read_sql(f"SELECT subscription_rock, subscription_energy FROM users WHERE user_id = '{update.effective_chat.id}';",engine)
    reply_keyboard = []
...
def edit_time_button(update, context, sms):#Поменять время...
    reply_keyboard = [['Поменять время смены КЗ'],
...
def new_button(update, context, sms):### Вывод кнопок
    reply_keyboard = [['🆘 Помощь 🆘'], ['Сколько у меня камней?', 'Полезная информация'], ['⚙️Настройка профиля⚙️']]
...
def setting_admin_button(update, context, sms):### Вывод кнопок админовских настроек
    reply_keyboard = [['Отправить напоминалку игроку ✏️✉️🧍‍♂️', 'Редактировать сообщение напоминалки 📝', 'Написать от имени бота🤖'],
...
def helpMy_button(update, context, sms):### Вывод кнопок помощи
    reply_keyboard = [['Инструкция по применению'],
...
def help_button(update, context, sms):### Вывод кнопок помощи
    reply_keyboard = [['Инструкция по КВ'],
