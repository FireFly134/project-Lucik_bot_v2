    ...
    if (
        ...
        if fighter[0] != fighter[1]:
            ...
            if fighter1 == fighter2:
                ...
                bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="Вы сражались очень долго, но потом устали и помирились! 🤝 НИЧЬЯ 🤝 \n\n Шансы на победу:\n"
                    + str(fighter[0])
                    + ": "
                    + str(fighter1)
                    + "% \n"
                ...
                video = open(
                    f"{working_folder}fight/fight" + str(num) + ".mp4", "rb"
                )
                bot.send_video(chat_id=update.effective_chat.id, video=video)
                bot.send_message(
                    chat_id=update.effective_chat.id,
        ...
        if chat_type == "supergroup" or chat_type == "group":
            ...
            bot.send_message(
                chat_id=update.effective_chat.id,
                text="Я тебя не понимаю, я еще не настолько продвинутый. Нажми помощь, и там будут инструкции по применению ;) (Вопросы и предложения можно написать ему: @Menace134)",
            )
