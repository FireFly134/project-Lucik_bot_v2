...
#########################################################
logging.basicConfig(filename=working_folder + 'send_msg_errors.log',
                    filemode='a',
                    level=logging.INFO,
...
def reminder_kz():
    ...
    info_clans = pd.read_sql(f"SELECT * FROM clans WHERE start = 'True' AND chat_id IS NOT NULL;", engine)
...
def reminder_zero(text="До обнуления камушков остался 1 час!"):
    info_clans = pd.read_sql(f"SELECT * FROM clans WHERE start = 'True' AND remain_zero_rock = 'True' AND chat_id IS NOT NULL;", engine)
    ### Напоминание в чат про смену КЗ ###
    for idx, row in info_clans.iterrows():
        t = Thread(target=send_msg, args=(row['chat_id'], text,))
        t.start()
...
def clear_rock():
    info_heroes_of_users = pd.read_sql(f"SELECT id FROM heroes_of_users WHERE rock > 0;",engine)
...
def reminder_energy():
    ...
    info_energy = pd.read_sql(f"SELECT user_id, name FROM heroes_of_users WHERE (time_collection_energy = '{int(time.strftime('%H'))}' OR time_collection_energy = '{int(time2_energy.strftime('%H'))}' OR time_collection_energy = '{int(time3_energy.strftime('%H'))}') AND subscription_energy = 'True';", engine)
    if len(info_energy) != 0:
        for idx, row in info_energy.iterrows():
            name = str(row['name']).replace('_', '\_').replace('*', '\*').replace('[', '\[').replace(']','\]').replace(
                '(', '\(').replace(')', '\)').replace('`', '\`').replace('~', '\~').replace('>', '\>').replace(
                '#', '\#').replace('+', '\+').replace('=', '\=').replace('-', '\-').replace('|', '\|').replace(
                '{', '\{').replace('}', '\}').replace('.', '\.').replace('!', '\!')
            t = Thread(target=send_msg_MarkdownV2, args=(row['user_id'], f"*Зайди в игру и забери халявную энергию*\. \({name}\)",))
            t.start()
            t.join()
