...
def main():
    ...
    sleep_time = datetime.strptime(f"{int(time.strftime('%H')) + 1}:00:00", "%H:%M:%S") - time
        ...
        if int(time.strftime("%M")) == 0:
            ...
            if len(info_energy) != 0:
                for idx, row in info_energy.iterrows():
                    ...
                    t = Thread(target=send_msg, args=(row['user_id'], f"*Зайди в игру и забери халявную энергию*\. \({name}\)",))
                    t.start()
                    t.join()
