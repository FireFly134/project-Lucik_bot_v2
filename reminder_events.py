...
def main():
    ...
    time_add_two_week = time + timedelta(days=16) #через 2 дня будет ивент и через 2 недели будет повтор поэтому 14+2=16
    ### Проверка на наличия энергии по времени для подписчиков ###
    info_event = pd.read_sql(f"SELECT * FROM check_events WHERE date = '{str(time_add_two_days)[:10]}';", engine)
    info_user = pd.read_sql("SELECT user_id FROM telegram_users_id;",engine)
    if len(info_event) != 0:
        engine.execute(f"UPDATE check_events SET date = '{str(time_add_two_week)[:10]} 00:00:00.0' WHERE date = '{str(time_add_two_days)[:10]}';")
        for idx, row in info_event.iterrows():
            text = str(row['description']).replace('_', '\_').replace('*', '\*').replace('[', '\[').replace(']','\]').replace(
                '(', '\(').replace(')', '\)').replace('`', '\`').replace('~', '\~').replace('>', '\>').replace(
                '#', '\#').replace('+', '\+').replace('=', '\=').replace('-', '\-').replace('|', '\|').replace(
                '{', '\{').replace('}', '\}').replace('.', '\.').replace('!', '\!')
            for idx, row_us in info_user.iterrows():
                t = Thread(target=send_msg, args=(str(row_us['user_id']), f"*Через два дня {text}*",))
                t.start()
                t.join()
