...
#########################################################
logging.basicConfig(
    filename=working_folder + "send_msg_errors_zero.log",
    filemode="a",
    level=logging.INFO,
...
def main():
    ...
    sleep_time1 = (
        datetime.strptime(f"{int(time.strftime('%H'))}:30:00", "%H:%M:%S")
        - time
    )
    sleep_time2 = (
        datetime.strptime(f"{int(time.strftime('%H')) + 1}:30:00", "%H:%M:%S")
        - time
    )
    ...
    while True:
        ...
        if int(time.strftime("%M")) == 30:
            ...
            info_clans = pd.read_sql(
                f"SELECT chat_id FROM clans WHERE start = 'True' AND chat_id IS NOT NULL;",
                engine,
            )

            info_kz = info_heroes_of_users[
                info_heroes_of_users["time_change_kz"]
                == int(time_kz.strftime("%H"))
            ]
            ...
            info_kz_info = info_kz_info[
                info_kz_info["description_of_the_kz"] == 1
            ]
            ...
            if len(info_kz_zero) != 0:
                ...
                list_to_zero = [
                    str(id_hero) for id_hero in info_kz_zero["id"].tolist()
                ]
                engine.execute(
            ...
            info_clans_kz = info_clans_kz[
                info_clans_kz["subscription_rock"] == 1
            ]
            ...
            info_clans_description_of_the_kz = (
                info_clans_description_of_the_kz[
                    info_clans_description_of_the_kz["description_of_the_kz"]
                    == 1
                ]
            )
            if len(info_clans_description_of_the_kz) != 0:
                ...
                if len(info) != 0:
                    for (
                        idx,
                        row,
                    ) in info_clans_description_of_the_kz.iterrows():
                        t = Thread(
                            target=send_msg_MarkdownV2,
                            args=(
                                row["chat_id"],
